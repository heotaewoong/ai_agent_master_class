...
def route_by_intent(state: NewsHubState):
    """
    ★ Conditional Edge — 의도에 따라 다음 노드를 결정한다.

    - "youtube"    → youtube_collector 만 실행
    - "news"       → news_searcher 만 실행
    - "both"       → Send API로 youtube_collector + news_searcher 병렬 팬아웃
    ...
    if intent == "youtube":
        ...
        return "curator"
    else:  # "both" → Send API로 병렬 팬아웃
        state_dict = dict(state)
        return [
            Send("youtube_collector", state_dict),
            Send("news_searcher", state_dict),
        ]
